...
namespace WinFormsApp2
{
    public class Enemy
    {
        ...
        public static readonly Image Image = new Bitmap(Path.Combine(
            new DirectoryInfo(Directory.GetCurrentDirectory()).Parent.Parent.Parent.FullName, "Models\\bigSlime.png"));
        ...
        public void Move()
        {
            var dirX = Player.CenterX - CenterPosX;
            var dirY = Player.CenterY - CenterPosY;
            PosX += dirX / (Math.Abs(dirX) + Math.Abs(dirY)) * Speed;
            PosY += dirY / (Math.Abs(dirX) + Math.Abs(dirY)) * Speed;
            CenterPosX += dirX / (Math.Abs(dirX) + Math.Abs(dirY)) * Speed;
            CenterPosY += dirY / (Math.Abs(dirX) + Math.Abs(dirY)) * Speed;
        }
